...
test_data = test_data[test_data['label']!= 4].copy() #N03 

num_classes=len(set(test_data['label'])) #Assumption: All classes represented in test set 
test_output_dir = model_output_dir + 'test_best_model_cuimc_evaluate_N03_tmp/'
os.makedirs(test_output_dir, exist_ok=True)
model_output_dir_cuimc = model_output_dir + 'test_cuimc/'+ input_suffix.split('/')[1].replace('.csv', '_current_'+model_output_dir.split('/')[-2])+'/'
...
au_roc_macro, pred_pos_dict, actual_pos_dict, f1_micro, class_specific_auroc_dict = get_metrics(y_pred, y_true,
                                                                ...
                                                                seed = seed, test_csv = '_N03_',
                                                                suffix='test_best_model_CUIMC',  plot=True)


#Save results
csv_dir = 'CUIMC/'+ input_suffix.split('/')[0].split('_')[1] + '/'
csv_dir = csv_dir.replace('N03X','N03')
conditions_info = input_suffix.split('/')[1].replace('N03X','N03')
